...
class Jadwal extends Model
{
  ...
  protected $fillable = ['hari_id', 'kelas_id', 'team_id', 'dosen_id', 'jam_mulai', 'jam_selesai', 'ruang_id'];
  ...
  public function ulangan($id)
  {
    ...
    $nilai = Ulangan::where('mhs_id', $mhs->id)->where('team_id', $id)->first();
    return $nilai;
  }
  ...
  {
    $mhs = Mhs::where('no_induk', Auth::user()->no_induk)->first();
    $nilai = Rapot::where('mhs_id', $mhs->id)->where('team_id', $id)->first();
    return $nilai;
  }
  ...
  public function cekUlangan($id)
  {
    ...
    $ulangan = Ulangan::where('mhs_id', $data['mhs'])->where('team_id', $data['team'])->first();
    return $ulangan;
  }
  ...
  {
    $data = json_decode($id, true);
    $rapot = Rapot::where('mhs_id', $data['mhs'])->where('team_id', $data['team'])->first();
    return $rapot;
  }
